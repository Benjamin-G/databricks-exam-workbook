...
def validate_and_clean_data(df):
    ...
    logger.info(f"After dropping nulls: {after_dropna} rows ({(initial_count-after_dropna)/initial_count*100:.2f}% removed)")
    ...
    logger.info(f"After filtering invalid data: {final_count} rows ({(after_dropna-final_count)/after_dropna*100:.2f}% removed)")
